...
interface HtmlNode {
  ...
  /** 节点原始 HTML 或文本内容。 */
  raw: string;
  /** 节点是否为纯文本或行内内容合并后的文本节点。 */
  text: boolean;
}

/** Gutenberg 列表块的 JSON 属性集合。 */
...
export function convertHtmlToGutenberg(html: string): string {
...
function readTopLevelNodes(html: string): HtmlNode[] {
...
/** 查找指定开始标签对应的闭合标签结束位置。 */
function findClosingTagEnd(html: string, tag: string, fromIndex: number): number {
...
function mergeInlineTextNodes(nodes: HtmlNode[]): HtmlNode[] {
...
function convertNodeToBlock(node: HtmlNode): string {
  if (node.text) {
...
function convertParagraph(raw: string): string {
...
/** 生成 Gutenberg 段落区块。 */
function paragraphBlock(paragraphHtml: string): string {
  return `<!-- wp:paragraph -->\n${paragraphHtml.trim()}\n<!-- /wp:paragraph -->`;
}

/** 生成 Gutenberg 标题区块，并确保标题标签包含 WordPress 标题类名。 */
function headingBlock(raw: string, tag: string): string {
...
function listBlock(raw: string): string {
...
function buildListBlockAttributes(tag: string, attrs: string): ListBlockAttributes {
  const blockAttrs: ListBlockAttributes = {};
  if (tag === "ol") {
...
/** 读取列表内部的顶层 li 节点。 */
function readTopLevelListItems(inner: string): string[] {
  const items: string[] = [];
  let index = 0;
...
function convertListItem(itemHtml: string): string {
...
function convertNestedListsInListItem(inner: string): string {
...
/** 生成 Gutenberg 图片区块。 */
function imageBlock(raw: string): string {
...
/** 生成 Gutenberg 引用区块。 */
function quoteBlock(raw: string): string {
...
/** 生成 Gutenberg 表格区块。 */
function tableBlock(raw: string): string {
...
function groupBlock(raw: string): string {
...
function ensureClass(raw: string, tag: string, className: string): string {
  const openTag = new RegExp(`^<${escapeRegExp(tag)}\\b([^>]*)>`, "i");
  return raw.replace(openTag, (match: string, attrs: string) => {
...
/** 从标签属性字符串中读取指定属性值。 */
function getAttributeValue(attrs: string, name: string): string {
...
/** 提取成对标签内部的 HTML。 */
function extractInnerHtml(raw: string, tag: string): string {
...
function normalizeVoidElement(raw: string, tag: string): string {
